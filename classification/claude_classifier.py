...
class ClaudeClassifier(BaseAIClassifier):
    ...
    def _call_ai_api(self, message: str, diff: str) -> dict:
        """Asks Claude for sub-categorization using Anthropic API.
...
Answer with ONLY the category name from the list above."""
    ...
    def run(self, input_file: str):
        ...
        print(f"Classifying {len(pairs) - len(processed_commits)} remaining pairs with Claude...")
        ...
        with open(input_file, 'w') as f:
            json.dump(pairs, f, indent=2)
        print(f"Updated {input_file} with Claude tag classifications")
